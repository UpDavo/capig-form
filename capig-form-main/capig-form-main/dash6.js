...
(function (global) {
  ...
  function buildDiagnosticosData() {
    ...
    // Procesar solo la hoja que concentra historico + nuevos (Django)
    ...
    const aliasToRuc = new Map(); // nombre normalizado -> ruc o clave placeholder
    ...
    base.rows.forEach((row) => {
      ...
      const key = ruc || razon;
      baseMap.set(key, { ruc, razonSocial: razon || "SIN_NOMBRE", tamano: tam || "SIN_TAMANO", sector: sector || "SIN CLASIFICAR" });
    ...
    // Default a 2024 para historico sin fecha y al maximo ano detectado (o 2024) para Django
    ...
    const histFallbackYear = DEFAULT_HIST_YEAR; // Historico sin fecha se ancla a 2024 para no mezclar con pruebas nuevas
    ...
    function resolveEmpresa(name) {
      ...
      const info = baseMap.get(norm);
      if (info) {
        aliasToRuc.set(norm, norm);
        return info;
      }

      const placeholder = { ruc: "", razonSocial: norm || "SIN_NOMBRE", tamano: "SIN_TAMANO", sector: "SIN CLASIFICAR", isPlaceholder: true };
      if (norm) {
    ...
    // Mapa para agregar Django por RUC|ANO|TIPO
    const djangoMap = new Map();

    // DIAGNOSTICOS - procesar TODO (historico sin fecha + nuevo con fecha)
    diagDjango.rows.forEach((row) => {
      const seDiagRaw = (getVal(row, diagDjango.headerIndex, ["SE_DIAGNOSTICO", "SE DIAGNOSTICO", "SE_DIAGNOSTICO_", "SE DIAGNOSTICO_"]) || "").toString().trim().toUpperCase();
      if (shouldSkipDiagnosticoFlag(seDiagRaw)) {
        filtNo++;
        return;
      }
      ...
      const tipo = tipoRaw || "SIN TIPO";
      // Filtro: No contar SIN TIPO / NINGUNO
      if (tipo === "SIN TIPO" || tipo === "NINGUNO") return;

      // IMPORTANTE: Solo nos enfocamos en TIPO DE DIAGNOSTICO
      // Tipos validos: LEAN, ESTRATEGIA, LEGAL, AMBIENTE, RRHH
      ...
      // CRITICAL FILTER: Distinguish diagnostics from legal advisories
      // - Non-legal types (LEAN, ESTRATEGIA, AMBIENTE, RRHH) = ALWAYS diagnostic
      // - Legal WITHOUT subtipo or with PENDIENTE = diagnostic
      // - Legal WITH specific subtipo (laboral, propiedad intelectual, etc.) = asesorias (EXCLUDE)
      if (tipo === "LEGAL") {
        const ADVISORY_LEGAL_SUBTYPES = ["LABORAL", "PROPIEDAD INTELECTUAL", "SOCIETARIO", "CONTACTO", "OTROS"];
        if (ADVISORY_LEGAL_SUBTYPES.includes(subtipoRaw)) {
          // This is a legal advisory, not a diagnostic
        ...
        // Legal with PENDIENTE or empty subtipo = valid diagnostic, continue processing
      }
      // All other types (LEAN, ESTRATEGIA, AMBIENTE, RRHH) = valid diagnostics, continue processing
      ...
      const razonDebug = getVal(row, diagDjango.headerIndex, ["RAZON_SOCIAL", "RAZON SOCIAL", "EMPRESA", "Razon Social"]);
      if (djangoProcesados < 5 || (hasDate && djangoProcesados < 15)) {
        Logger.log(`[DASH6 INCLUIDO] ${razonDebug} | Tipo: ${tipo} | Subtipo: ${subtipoRaw || "vacio"} | Fecha: ${hasDate ? yearFromDate : "sin fecha"}`);
      }

      // IMPORTANT: Process ALL rows (with and without dates)
      // Rows without dates = historical data, rows with dates = new data
      ...
      // Determine year: if no date, it's historical data (2024), otherwise use actual year
      const anio = yearOrDefault(fechaRaw, hasDate ? "DJANGO" : "HIST", hasDate ? djangoFallbackYear : histFallbackYear);
      if (!hasDate) sinFechaRecuperados++;

      // Clave unica para agregacion
      const key = `${empresa.ruc || empresa.razonSocial}|${anio}|${tipo}`;

      let bucket = djangoMap.get(key);
      if (!bucket) {
        bucket = {
          ruc: empresa.ruc,
    ...
    // HISTORICAL SHEET PROCESSING SKIPPED
    // All historical data is already in DIAGNOSTICOS (rows without dates)
    Logger.log("[DASH6] Hoja historica DIAGNOSTICO no procesada (datos ya en DIAGNOSTICOS)");
    ...
    Logger.log(`Duplicados eliminados: ${duplicadosEliminados}`);
    Logger.log(`Total diagnosticos: ${diagnosticos.length}`);

    const yearsList = Array.from(years).sort((a, b) => yearRank(b) - yearRank(a));
    Logger.log(`Anos detectados: ${yearsList.join(", ")}`);

    generateDiagnosticosResumenTable(baseMap, diagnosticos, yearsList);
  ...
  function generateDiagnosticosResumenTable(baseMap, diagnosticos, years) {
    const rows = [];
