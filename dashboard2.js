...
const Dash2Module = (function () {
  ...
  const DASH2_SHEETS = {
    ...
    OUT_LATEST: "DASH_CAMBIO_TAMANO_ULTIMO",
  ...
  function buildTransitions(detailRows) {
    ...
    function pickLatestTransitionPair(rows) {
      const pairs = rows
        .map((r) => [parseInt(r[0], 10), parseInt(r[1], 10)])
        .filter(([yearInitial, yearFinal]) => !isNaN(yearInitial) && !isNaN(yearFinal) && yearFinal > yearInitial);
      if (!pairs.length) return null;
      const consecutive = pairs.filter(([yearInitial, yearFinal]) => yearFinal === yearInitial + 1);
      const source = consecutive.length ? consecutive : pairs;
      source.sort((a, b) => b[1] - a[1] || b[0] - a[0]);
      return source[0];
    }

    const latestPair = pickLatestTransitionPair(pivotRows);
    const latestInitialYear = latestPair ? String(latestPair[0]) : "2022";
    const latestFinalYear = latestPair ? String(latestPair[1]) : "2023";

    const rows2023 = pivotRows.filter(
      (r) => r[0] === latestInitialYear && r[1] === latestFinalYear && Math.abs(r[9]) === 1
    );
    ...
    const shLatest = getSheetOrCreate(DASH2_SHEETS.OUT_LATEST);
    shLatest.clear();
    shLatest.getRange(1, 1, out2023.length, headers2023.length).setValues(out2023);
    shLatest.autoResizeColumns(1, headers2023.length);

    // VersiÃ³n completa (incluye saltos de mÃ¡s de 1 nivel) para referencias tipo PPT.
    const rows2023Full = pivotRows.filter((r) => r[0] === latestInitialYear && r[1] === latestFinalYear);
    ...
    const shLatestFull = getSheetOrCreate("DASH_CAMBIO_TAMANO_ULTIMO_FULL");
    shLatestFull.clear();
    shLatestFull.getRange(1, 1, out2023Full.length, headers2023Full.length).setValues(out2023Full);
    shLatestFull.autoResizeColumns(1, headers2023Full.length);
